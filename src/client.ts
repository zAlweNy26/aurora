...
import { createHash, randomUUID, type BinaryLike, createSign, X509Certificate, createPrivateKey, KeyObject } from 'node:crypto'
...
export default class PSD2Client {
    #client: OAuth2Client
    #identifier: string
    #certificate: string
    #key: KeyObject
    ...
    private calculateSignature(headers: Record<string, any>) {
        ...
        const signingString = headersToSign.map(key => `${key.toLowerCase()}: ${headers[key]}`).sort().join('\n')
    ...
    async checkSCA(consentId: string, authorisationId: string) {
        return (await this.fetch<{ scaStatus: ScaStatus }>(`/consents/${consentId}/authorisations/${authorisationId}`)).scaStatus
    }

    async fetch<T, R extends ResponseType = "json">(path: string, options?: Omit<FetchOptions, 'baseURL' | 'agent'> & { headers?: Record<string, any> }): Promise<MappedResponseType<R, T>> {
